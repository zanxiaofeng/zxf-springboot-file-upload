...
- [四、三种方式对比总结](#四三种方式对比总结)
- [五、为什么 Base64 内嵌 JSON 无法流式处理](#五为什么-base64-内嵌-json-无法流式处理)
- [六、流式处理内存常数级的根本原因](#六流式处理内存常数级的根本原因)
- [七、工程实践准则](#七工程实践准则)
- [八、常见误区与踩坑清单](#八常见误区与踩坑清单)

---

## 一、核心结论

HTTP 协议本身对消息体（body）是**透明的字节流**——协议层并不区分"文本"和"二进制"。两者的差异体现在三个层面：

1. **Content-Type**：声明 body 的解释方式；
2. **编码方式**：文本需要字符集编解码，二进制原样传输或 Base64 文本化；
3. **读取方式**：文本通常全量解码为字符串，二进制可以逐块流式消费。
...
**特点**：零编解码开销、保真、体积最小，是传输文件、图片、ProtoBuf 序列化数据的首选。
...
| 解码出的 String / JsonNode | ~133MB（Java String 内部 char[]/byte[] 可能更多） |
| Base64.decode 后的 byte[] | 100MB |
| GC 前的临时数组 | 若干 |

**3~5 倍放大**，大文件直接 OOM——这正是"大文件不能这么传"的硬约束。

### 5.5 原因四：流式解码的"起点"被锁死
...
2. **文本漏写 charset**：`application/json` 默认按 UTF-8，但 `text/*` 某些场景默认 ISO-8859-1，导致中文乱码。显式声明 `charset=UTF-8`。
